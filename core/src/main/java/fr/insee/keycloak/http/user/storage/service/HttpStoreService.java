...
public class HttpStoreService {
  ...
  public HttpStoreService(
    ...
    mappers =
        multivaluedHashMap.getList(HttpStorageConstants.MAPPERS_MAP).stream()
            .collect(Collectors.toMap(s -> s.split(";")[0], s -> s.split(";")[1]));
    idAttribute = multivaluedHashMap.getFirst(HttpStorageConstants.ID_ATTRIBUTE);

    validateCredentialsPath =
        multivaluedHashMap.getFirst(HttpStorageConstants.PATH_VALIDATE_CREDENTIALS);
    validateCredentialsMethod =
        multivaluedHashMap.getFirst(HttpStorageConstants.METHOD_VALIDATE_CREDENTIALS);
    validateCredentialsHeader =
        multivaluedHashMap.getList(HttpStorageConstants.HEADERS_VALIDATE_CREDENTIALS).stream()
            .collect(Collectors.toMap(s -> s.split("=")[0], s -> s.split("=")[1]));
    validateCredentialsBodyType =
        multivaluedHashMap.getFirst(HttpStorageConstants.BODY_TYPE_VALIDATE_CREDENTIALS);
    validateCredentialsBody =
        multivaluedHashMap.getList(HttpStorageConstants.BODY_VALIDATE_CREDENTIALS);
    ...
    getByUsernameHeader =
        multivaluedHashMap.getList(HttpStorageConstants.HEADERS_GET_BY_USERNAME).stream()
            .collect(Collectors.toMap(s -> s.split("=")[0], s -> s.split("=")[1]));
    getByUsernameBodyType =
        multivaluedHashMap.getFirst(HttpStorageConstants.BODY_TYPE_GET_BY_USERNAME);
    ...
    getByIdHeader =
        multivaluedHashMap.getList(HttpStorageConstants.HEADERS_GET_BY_ID).stream()
            .collect(Collectors.toMap(s -> s.split("=")[0], s -> s.split("=")[1]));
    ...
    getByEmailHeader =
        multivaluedHashMap.getList(HttpStorageConstants.HEADERS_GET_BY_EMAIL).stream()
            .collect(Collectors.toMap(s -> s.split("=")[0], s -> s.split("=")[1]));
    ...
    updateCredentialMethod =
        multivaluedHashMap.getFirst(HttpStorageConstants.METHOD_UPDATE_CREDENTIAL);
    updateCredentialHeader =
        multivaluedHashMap.getList(HttpStorageConstants.HEADERS_UPDATE_CREDENTIAL).stream()
            .collect(Collectors.toMap(s -> s.split("=")[0], s -> s.split("=")[1]));
    updateCredentialBodyType =
        multivaluedHashMap.getFirst(HttpStorageConstants.BODY_TYPE_UPDATE_CREDENTIAL);
  ...
  public UserModel getUserFromJson(JsonNode json) {
    ...
    return new HttpUserModelAdapter(
        session, session.getContext().getRealm(), userStorageModel, user);
  }
}
